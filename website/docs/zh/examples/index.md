...
| `examples/threading-regressions` | 一个 checkout 页面，故意在 render 里调用 native module，使用 `useLayoutEffect`，并遗漏 `main-thread:` handler directive。 | Threading、Lifecycle、Events 三类 error |
| `examples/event-mode-settings` | 一个 settings 页面，开启 `globalPropsMode: "event"` 后仍直接读取 `lynx.__globalProps`，并且 lazy 组件没有 `Suspense`。 | Configuration 和 Performance 两个 warning |
